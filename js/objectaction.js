...
// 遍历对象 将二层布尔值换成数字
async function objectTravelsal() {
    let checkResult = {
        cellPhoneInfoCheck: { inTime: 1, availableBalance: 1 },
        keyInfoCheck:
        {
            operatorInfoMatch: 1,
            basicInfoCrossCheck: -1,
            blackListCheck: 1,
            contactsBlackListCheck1: -1,
            contactsBlackListCheck2: -1
        },
        activeCheck: { activeCheck: 1, info: '' },
        riskContactCheck: { riskContactCheck: 1, info: '' },
        contactsCheck:
        {
            validContactsNumCheck: 1,
            validEmergencyContactsCheck: 1,
            nonlocalPhoneCallCheck: 1
        },
        plusItems:
        {
            consumptionPlus: -1,
    ...
    const list = Object.keys(checkResult)
    await list.forEach((element) => {
        const list2 = Object.keys(checkResult[element])
        list2.forEach((value) => {
            if (typeof (checkResult[element][value]) === 'boolean') {
                checkResult[element][value] = sbfunc(checkResult[element][value])
            }
        })
    })
    return checkResult
...
export {objectArrayToObject, objectFilter}
